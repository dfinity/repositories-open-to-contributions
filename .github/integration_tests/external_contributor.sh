...
git merge upstream/master integration-test-$RUN_NUMBER
...
# gh pr create --repo dfinity/repositories-open-to-contributions --base master --head integration-test-$RUN_NUMBER --draft --title "Integration Test 3" --body "generated by integration test from branch $GITHUB_HEAD_REF"
