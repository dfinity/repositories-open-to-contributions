...
# create pull request
git push --set-upstream upstream integration-test-$RUN_NUMBER
gh auth login --with-token <<< $AUTH_TOKEN
gh pr create --repo dfinity/repositories-open-to-contributions --base $CURRENT_BRANCH --head integration-test-$RUN_NUMBER --draft --title "Integration Test 3" --body "generated by integration test from branch $GITHUB_HEAD_REF"
